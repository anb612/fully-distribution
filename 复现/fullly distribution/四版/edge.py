...
class Edge:
    ...
    def trigger_function(self):
        ...
        phi_ij = self.alpha_i / (2 * d_i) * self.zeta_ij @ self.hz_ij.T @ self.Gamma @ self.hz_ij + self.zeta_ij @ self.hz_ij.T @ self.Gamma @ self.e_ij - self.theta_ij @ self.e_ij.T @ self.Gamma @ self.e_ij
        phi_ji = self.alpha_j / (2 * d_j) * self.zeta_ji @ self.hz_ji.T @ self.Gamma @ self.hz_ji + self.zeta_ji @ self.hz_ji.T @ self.Gamma @ self.e_ji - self.theta_ji @ self.e_ji.T @ self.Gamma @ self.e_ji
        # if self.rho_ij >= 0 or self.rho_ji >= 0:
        #     return False
        # if (phi_ij <= 0 and self.rho_ij <= 0) or (phi_ji <= 0 and self.rho_ji <= 0) :
        #     return True
        # else:
        #     return False
        if (phi_ij <= 0 and self.rho_ij <= 0):
            self.sleep_time = self.sleep_ij
            self.indicator_ij = 0
            self.indicator_ji = 0
            return True
        elif (phi_ji <= 0 and self.rho_ji <= 0):
            self.sleep_time = self.sleep_ji
            self.indicator_ij = 0
            self.indicator_ji = 0
            return True
        else:
    ...
    def sleepping_mechnism(self):
        ...
        if self.rho_ij <= 0:
            ...
            sign_rho_ij = -np.sign(self.rho_ij) * (self.Gamma_max / self.P_min)
            term_1 =  ((d_i / self.alpha_i) * self.zeta_ij + d_i * (d_i - 1) * zeta_i_bar) * self.rho_ij**2
            term_2 = 2 * ((d_i / self.alpha_i) * self.zeta_ij + 1) * self.rho_ij + (d_i / self.alpha_i) * self.zeta_ij + 2 * self.theta_ij
            rho_ij_decay_rate = sign_rho_ij * (term_1 + term_2)
        
        if self.rho_ji <= 0:
            rho_ji_decay_rate = 0
        else:
            sign_rho_ji = -np.sign(self.rho_ji) * (self.Gamma_max / self.P_min)
            term_1 =  ((d_j / self.alpha_j) * self.zeta_ji + d_j * (d_j - 1) * zeta_j_bar) * self.rho_ji**2
            term_2 = 2 * ((d_j / self.alpha_j) * self.zeta_ji + 1) * self.rho_ji + (d_j / self.alpha_j) * self.zeta_ji + 2 * self.theta_ji
    ...
    def update_z(self):
        self.z_ij = self.head.position - self.tail.position
        self.z_ji = -self.z_ij
        self.hz_ij = self.z_ij
        self.hz_ji = -self.hz_ij
